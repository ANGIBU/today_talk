...
from services.user_service import check_duplicate  # 중복 확인 서비스 추가
from db import db  # db 객체 임포트 추가
import logging
...
def register():
    if request.method == 'POST':
        try:
            ...
            username = request.form.get('username', '').strip()
            password = request.form.get('password', '').strip()
            nickname = request.form.get('nickname', '').strip()  # 닉네임 추가
            ...
            if result:
                ...
                return redirect(url_for('auth.login'))  # 성공 시 로그인 페이지로 리다이렉트
            else:
